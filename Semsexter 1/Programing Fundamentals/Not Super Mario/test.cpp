...
void updateRender(char level[][levelColumns], char render[][renderWidth]);
void drawFrame(char render[][renderWidth]);
char movePlayer(char level[][levelColumns], string direction, string &levelNo);
void gotoxy(int x, int y);
void hideCursor();
bool isGrounded(char level[][levelColumns]);
void clearScreen();
...
int main() { 
    ...
    int enemyX[MAX_ENEMIES] = {23, 23, 23};   
    int enemyY[MAX_ENEMIES] = {115, 165, 275}; 
    int enemyDir[MAX_ENEMIES] = {1, -1, 1};  
    int enemyCount = 3;
    ...
    while (gameRunning) {
        ...
        if ((GetAsyncKeyState('A') & 0x8000) || (GetAsyncKeyState('D') & 0x8000)) {
            ...
            if (movePlayer(level, direction, levelNo) == '"') {
        ...
        // Jump
        if ((GetAsyncKeyState('W') & 0x0001) && isGrounded(level)) {
        ...
        while (playerY != newY) {
            ...
            if (getCharAtLevel(level, playerX, playerY + step) == '#' || 
                ...
                if (getCharAtLevel(level, playerX, playerY + step) == 'M' && step < 0) score++;
                velocityY = 0;
